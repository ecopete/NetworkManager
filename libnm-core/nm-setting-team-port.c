 ...
 **/
void
nm_setting_team_port_remove_link_watcher (NMSettingTeamPort *setting, int idx)
{
	NMSettingTeamPortPrivate *priv = NM_SETTING_TEAM_PORT_GET_PRIVATE (setting);
              ...
              const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_LINK_WATCHERS:
		...
		if (priv->link_watchers->len > 1)
			align_value = value;
		else if (priv->link_watchers->len == 1) {
			NMTeamLinkWatcher *watcher;

			watcher = nm_team_link_watcher_new (NM_TEAM_LINK_WATCHER_ETHTOOL,
			                                    0, 0, 0, NULL, NULL, 0, NULL);
			if (!nm_team_link_watcher_equal (priv->link_watchers->pdata[0], watcher))
				align_value = value;
