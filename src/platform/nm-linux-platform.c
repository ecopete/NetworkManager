...
static gboolean
qdisc_delete (NMPlatform *platform, const NMPObject *obj)
{
	struct nl_msg *msg;

	msg = _nl_msg_new_qdisc (RTM_DELQDISC, 0, NMP_OBJECT_CAST_QDISC (obj));
	if (!msg)
		g_return_val_if_reached (FALSE);
	return do_delete_object (platform, obj, msg);
}
...
static gboolean
tfilter_delete (NMPlatform *platform, const NMPObject *obj)
{
	struct nl_msg *msg;

	msg = _nl_msg_new_tfilter (RTM_DELTFILTER, 0, NMP_OBJECT_CAST_TFILTER (obj));
	if (!msg)
		g_return_val_if_reached (FALSE);
	return do_delete_object (platform, obj, msg);
}
